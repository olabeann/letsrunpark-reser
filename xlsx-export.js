(function(root){
  ...
  function styles(ns) {
    ...
    // Uniform 11pt font; gray headers, yellow amounts, blue totals and thin table borders.
    return '<styleSheet xmlns="'+ns+'"><numFmts count="1"><numFmt numFmtId="164" formatCode="#,##0;[Red](#,##0);&quot;—&quot;"/></numFmts>' +
      '<fonts count="1">'+font(11,'FF000000',false)+'</fonts>' +
      '<fills count="5"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill>'+fill('FFD9D9D9')+fill('FFFFFF00')+fill('FFE2EFF5')+'</fills>' +
      ...
      '<cellXfs count="13">'+xf(0,0,false,'left',false)+xf(0,2,false,'center')+xf(0,0,true)+xf(0,0,false)+xf(0,0,true)+xf(0,4,false)+xf(0,4,true)+xf(0,0,false,'left',false)+xf(0,0,false,'left',false)+xf(0,3,false,'center')+xf(0,3,true)+xf(0,0,false,'center')+xf(0,0,false,'center')+'</cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>';
  }
  ...
  function worksheet(sheet, ns) {
    ...
    var data=rows.map(function(row,index){
      ...
      return '<row r="'+rn+'">'+row.map(function(value,ci){
        var numeric=typeof value==='number', style=numeric?2:3;
        ...
        if(!header && ci===layout.statusCol)style=String(value).includes('취소')?12:11;
        if(!inTable)style=0;
        var ref=col(ci)+rn;
        if(value===null || value===undefined)return '<c r="'+ref+'" s="'+style+'"/>';
        return numeric?'<c r="'+ref+'" s="'+style+'"><v>'+value+'</v></c>':'<c r="'+ref+'" s="'+style+'" t="inlineStr"><is><t xml:space="preserve">'+xml(value)+'</t></is></c>';
      }).join('')+'</row>';
    }).join('');
    ...
    return '<worksheet xmlns="'+ns+'"><sheetPr><pageSetUpPr fitToPage="1"/></sheetPr><dimension ref="A1:'+col(width-1)+rows.length+'"/>' +
      ...
      (layout.filter && rows.length>1?'<autoFilter ref="A'+headers[0]+':'+col(width-1)+rows.length+'"/>':'') +

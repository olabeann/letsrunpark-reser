(function () {
    ...
    frontend: {
      ...
      groups: [
        { title: "사용자 예약 화면", eyebrow: "CUSTOMER FLOW", items: [
          ...
          ["티켓 상태 · 정보 표시", "티켓 목록 카드와 상세 화면 진입", "1초 주기로 현재 시각과 회차 시작·종료 시각을 비교해 상태를 다시 계산합니다.", "카드에는 상태 배지(노란색 ‘입장 대기’·초록색 ‘입장 가능’·회색 ‘입장 종료’), 대표 이미지, 프로그램명, 이용일·회차, 인원수, 결제금액과 과천시민 할인 여부를 표시합니다. 상세 화면은 여기에 더해 현재 시각, 입장 안내 문구, 예약번호, 입장 시작·회차·마감 시각과 과천시민 할인 인원의 증빙 확인 배지를 함께 보여줍니다.", "색만으로 상태를 구분하지 않도록 배지에는 항상 텍스트 라벨을 함께 표시합니다."],
          ["티켓 취소 · 입장", "티켓의 ‘예약 취소’ 또는 입장 상태를 확인", "본인 예약, 유효 인원, 프로그램별 취소 마감과 현재 취소·환불·입장 상태를 확인합니다.", "입장 대기 상태이면서 관리자 설정 취소 마감 전일 때만 인원별 정상가·할인 티켓을 선택해 전체 또는 부분취소합니다. 입장 상태값은 입장 대기·입장 가능·입장 종료를 그대로 유지합니다.", "환불 내역은 결제 영역 하단에 기록하고 카드사 영업일 기준 5~7일이 걸릴 수 있음을 안내합니다. 나눠 입장과 증빙 실패 처리는 별도 정책을 따릅니다."],
          ["회원탈퇴", "‘내 티켓’ 화면의 ‘회원탈퇴’를 클릭", "시스템 확인 모달로 탈퇴 의사를 다시 확인합니다.", "확인하면 유효한 예약이 남아 있어도 제한 없이 즉시 탈퇴 처리하고, 재가입 제한 없이 같은 로그인 제공자로 바로 다시 가입할 수 있습니다.", "탈퇴 후 결제·이용 이력의 보관 기간과 파기 절차는 개인정보 처리방침에 따로 정의합니다."],
        ...
        { title: "운영 관리자 화면", eyebrow: "ADMIN FLOW", items: [
          ...
          ["프로그램 저장", "‘새 프로그램’ 또는 ‘수정’ 후 저장을 클릭", "명칭, 지역·부서, 운영/노출 기간, 요일, 가격, 대표 이미지, 예약 가능 일수, 취소 마감, 적용 할인 값을 검증합니다.", "저장 후 해당 프로그램의 회차 관리 화면으로 이동합니다.", "종료일이 시작일보다 빠른 값이나 예약 가능 일수 1일 미만 등 잘못된 값은 저장하지 않고 오류를 안내합니다."],
          ...
          ["내보내기 · 정산", "정산 행을 열거나 ‘거래 원장 CSV’를 클릭", "현재 필터, 정산 권한, 서비스 완료일, 지역·부서·프로그램 범위를 적용합니다.", "요약과 일별 집계 아래에 예약·결제·환불을 1건 단위 거래 원장으로 표시하고 현재 검색·유형 필터 범위를 내려받습니다.", "개별 원장에는 예약번호·결제번호·인원·결제액·환불액·PG 수수료·지급 예정액·상태를 포함하며 개인정보 다운로드 이력을 남깁니다."]
        ]}
    ...
    backend: {
      ...
      groups: [
        ...
        { title: "관리 · 운영 API", eyebrow: "ADMIN API", items: [
          ...
          ["할인 정책 저장", "할인 추가/수정/비활성화", "정률·정액, 상한, 적용 수량, 기간, 프로그램과 권한을 검증합니다. 수량 한도는 계정당 이용일마다 합산하고 증빙은 현장 확인으로 고정합니다.", "정책과 프로그램 관계 데이터로 저장해 신규 견적에 반영합니다.", "과거 주문은 스냅샷을 사용하며 과천시민 고정값 변경은 별도 상위 권한이 필요합니다."],
          ...
          ["정산 집계", "기간별 정산 조회 또는 파일 생성", "서비스 완료일 기준으로 환불 완료액과 PG 수수료를 반영해 지역·부서·프로그램별 집계합니다.", "요약·일별 집계와 함께 금액을 구성하는 불변 거래 원장을 예약·결제번호 기준으로 반환합니다.", "환불은 원 결제를 덮어쓰지 않고 별도 차감 거래로 연결하며 지급일·예외 차감 방식은 확정 전까지 ‘예상’으로 구분합니다."],
  ...
  var servicePolicy = {
    ...
    "RSV-07": [
      ...
      ["취소 · 입장", "사용자 취소 또는 현장 입장", "관리자가 프로그램별로 설정한 취소 마감 전이며 입장 대기 상태인 예약만 취소할 수 있습니다.", "묶음 티켓은 취소 시 인원별 정상가·할인 티켓으로 펼쳐지고 선택한 1명 이상의 실제 결제액만 환불합니다. 환불 내역은 결제 영역 아래에 기록합니다.", "카드사 환불 완료에는 영업일 기준 5~7일이 걸릴 수 있습니다. 나눠 입장, 노쇼와 현장 증빙 실패는 D-04~D-05에서 확정합니다."],
    ...
    "RSV-08": [
      ["회원탈퇴", "탈퇴를 원하는 로그인 회원", "시스템 확인 모달로 탈퇴 의사를 확인하며, 유효한 예약이 있어도 탈퇴를 제한하지 않습니다.", "탈퇴 시 예약·장바구니를 포함한 계정 전체 내역을 삭제하고, 재가입 제한 없이 같은 로그인 제공자로 즉시 다시 가입할 수 있습니다.", "탈퇴 후 결제·이용 이력의 보관 기간과 파기 절차는 개인정보 처리방침에 따로 정의합니다."]
    ],
    "RSV-09": [
    ...
    "ADM-03": [["회차 운영", "반복 회차와 날짜별 실제 회차", "온라인 정원은 현장 키오스크 수량과 독립적으로 관리합니다.", "기존 예약이 있는 회차 변경은 영향과 재고를 확인한 뒤 처리합니다.", "정원 축소·공유 자원·재판매 시점은 D-06에서 확정합니다."], ["운영일 · 휴장일", "지역과 전체 또는 개별 프로그램의 단일 날짜·기간", "캘린더에는 프로그램에 이미 등록된 운영일을 표시하고, 예약을 닫을 휴장일만 별도 저장합니다.", "전체 프로그램 휴장은 해당 지역의 모든 예약을 차단하고 개별 프로그램 휴장은 선택한 프로그램에만 우선 적용합니다.", "이미 예약된 날짜를 휴장으로 바꿀 때는 대상 예약 취소·환불 절차를 별도로 실행해야 합니다."]],

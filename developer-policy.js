(function () {
    ...
    frontend: {
      ...
      groups: [
        { title: "사용자 예약 화면", eyebrow: "CUSTOMER FLOW", items: [
          ...
          ["인원 · 할인", "인원 +/− 또는 할인을 클릭", "일반 구매 수량에는 일일 한도를 두지 않고 회차의 판매 가능 수량만 적용합니다. 할인은 정책별 일일 한도를 별도로 적용합니다.", "수량과 할인 선택에 따라 결제 예정 금액과 잔여 수량을 다시 계산합니다.", "온라인에서 별도 할인 증빙을 수집하거나 검증하지 않습니다."],
          ["장바구니 담기", "날짜·회차·인원 선택 후 ‘장바구니 담기’를 클릭", "로그인, 필수 선택값, 동일 이용일·지역·부서, 전 지역 예약 시간 중복과 할인 사용량을 검사합니다.", "통과하면 해당 수량의 재고를 10분간 점유하고 만료 시각과 함께 장바구니를 갱신합니다.", "장바구니 수량 변경 시 10분을 다시 계산하며 삭제·결제·만료 시 점유 재고를 즉시 해제합니다."],
        ...
        { title: "운영 관리자 화면", eyebrow: "ADMIN FLOW", items: [
          ["예약 검색 · 상세", "검색 버튼을 클릭하거나 검색창에서 Enter, 필터 변경 또는 예약 행 클릭", "검색어 앞뒤 공백을 제거한 뒤 예약번호·결제번호·프로그램명에서 부분 일치로 찾고 지역→부서, 이용일, 프로그램, 예약 상태를 AND 조건으로 적용합니다.", "검색 실행 시 첫 페이지부터 다시 조회하고 결과 건수와 적용 조건을 유지합니다. 초기화는 검색어·모든 필터·페이지를 기본값으로 되돌린 뒤 즉시 전체 목록을 다시 조회합니다.", "빈 결과에는 ‘조건에 맞는 예약이 없습니다’와 초기화 버튼을 표시합니다. 다른 부서 데이터의 상세·취소·다운로드는 권한에 따라 각각 제한합니다."],
          ...
          ["프로그램 표시 · 판매 상태", "프로그램 목록을 조회하거나 ‘판매중/숨김’을 클릭", "목록의 할인 정책은 우선순위가 가장 높은 할인명을 먼저 표시하고 2개 이상이면 ‘과천시민 할인 외 N개’, 없으면 ‘없음’으로 표시합니다. 등록 회차는 전체 수와 판매중 수를 구분합니다.", "판매중이면 사용자 화면에 노출하고 숨김이면 신규 예약 화면에서 제외합니다. 상태 변경 성공 후 해당 행만 서버 응답으로 갱신합니다.", "상태 변경은 기존 예약·결제 스냅샷에 영향을 주지 않습니다. 실패 시 기존 배지를 유지하고 오류를 안내하며 낙관적 상태를 확정하지 않습니다."],
          ...
          ["회차 저장 · 상태 변경", "‘회차 관리’에서 등록/수정/판매중/숨김 클릭", "시작·종료 시각, 판매 수량, 시간 중복과 프로그램 권한을 검증합니다. 종료 시각은 시작보다 늦어야 하고 판매 수량은 1명 이상이어야 합니다.", "회차는 시작 시각순으로 정렬하고 회차 번호를 자동 부여합니다. 저장 성공 시 목록과 사용자 예약 가능 회차를 다시 조회합니다.", "기존 예약 수보다 판매 수량을 낮추거나 예약이 있는 회차의 시간·상태를 바꿀 때 영향 인원과 차단 사유를 먼저 보여줍니다. 현장 재고는 이 값과 합산하지 않습니다."],
          ["할인 목록 · 저장", "‘할인 관리’ 열기, 할인 추가/수정/활성 상태 저장", "할인명, 정률 1~100% 또는 정액 1원 이상, 적용 기간, 최대 수량과 대상 프로그램을 검증합니다. 목록은 대표 할인명+‘외 N개’로 요약합니다.", "활성 할인만 신규 견적에 사용하고 취소·환불 완료 시 할인 티켓 수만큼 이용일 한도를 복원합니다. 온라인 증빙 항목은 두지 않습니다.", "MVP는 한 티켓에 할인 하나만 적용하고 과거 주문은 결제 당시 할인 스냅샷을 유지합니다."],
          ["선택 티켓 취소", "예약 상세에서 유효 인원 1명 이상 선택 후 ‘선택 취소·환불’ 클릭", "취소 가능 상태, 선택 티켓, 취소 마감, 실제 배분 결제액, 환불 권한과 사유를 서버에서 다시 확인합니다.", "예약 상태는 ‘예약 확정·취소 처리 중·부분 취소·취소 완료’로만 표시합니다. 처리 이력에는 고객 직접 취소/관리자 처리, 처리자, 시각, 대상, 금액과 사유를 기록합니다.", "PG 응답 불명확·일부 실패는 예약 상태를 ‘취소 처리 중’으로 두고 상세 환불 내역에 ‘환불 확인 필요’를 별도로 표시합니다."],
          ["운영일 · 휴장일", "지역과 전체/개별 프로그램을 선택하거나 휴장일 등록", "캘린더에는 프로그램에 등록된 운영일만 표시하고 회차는 노출하지 않습니다. 전체 프로그램 휴장은 개별 프로그램 운영보다 우선하며 같은 날짜의 중복 휴장 등록은 막습니다.", "필터 변경 시 선택 월을 유지해 다시 조회하고 휴장 등록 성공 후 해당 날짜만 갱신합니다. 휴장 해제 시 원래 운영 일정에 포함되는 날짜만 다시 운영으로 표시합니다.", "이미 예약이 있는 날짜는 휴장 저장만으로 자동 환불하지 않습니다. 대상 예약 수와 별도 운영 취소 절차를 안내합니다."],
    ...
    backend: {
      ...
      groups: [
        { title: "예약 · 결제 API", eyebrow: "BOOKING API", items: [
          ...
          ["장바구니 견적 · 재고 점유", "담기/수량 변경/주문 화면 진입 시 견적 요청", "상품·회차 ID, 수량, 동일 이용일·지역·부서, 전 지역 시간 중복, 활성 할인과 사용량을 서버 데이터로 계산합니다.", "재고 점유 토큰과 서버 기준 만료 시각을 반환하며 점유 시간은 마지막 장바구니 수량 변경부터 10분입니다.", "만료·삭제·결제 완료 시 재고를 해제하고, 만료된 장바구니의 결제 요청은 409와 재선택 안내를 반환합니다."],
          ...
          ["취소 · 환불", "사용자 또는 관리자가 전체/부분 취소 요청", "예약 상태와 결제·환불 상태를 분리해 소유권/권한, 취소 마감, 티켓 상태, 환불 가능액, PG 취소 가능 여부를 검사합니다.", "선택 티켓 금액만 PG 부분취소하고 예약 상태, 환불 누계, 처리자·사유·시각을 기록합니다.", "PG 결과가 불명확하면 예약은 ‘취소 처리 중’, 환불 상태는 ‘환불 확인 필요’로 저장해 재조회합니다."],
          ["할인 사용량 복원", "취소·환불 완료 후", "취소가 완료된 할인 티켓 수를 이용일별 할인 사용량에서 차감합니다.", "환불 성공과 같은 트랜잭션에서 일일 할인 한도를 즉시 복원합니다.", "환불 확인 필요 상태에서는 중복 사용을 막기 위해 복원하지 않고 환불 완료 후 복원합니다."],
        ...
        { title: "관리 · 운영 API", eyebrow: "ADMIN API", items: [
          ...
          ["할인 정책 저장 · 요약", "할인 추가/수정/비활성화 또는 프로그램 목록 조회", "정률·정액, 우선순위, 상한, 적용 수량, 기간, 프로그램과 권한을 검증하며 온라인 증빙 항목은 두지 않습니다.", "정책의 일일 한도를 이용일 기준으로 집계하고 취소·환불 완료 시 해당 티켓 수량만큼 복원합니다. 목록에는 ‘할인명 외 N개’로 요약합니다.", "MVP는 한 티켓에 하나의 할인만 허용하고 과거 주문은 할인 스냅샷을 사용합니다."],
        ...
        { title: "아직 확정이 필요한 정책", eyebrow: "DECISION REQUIRED", pending: true, items: [
          ["D-01 상품 운영값", "상품 등록·예약 노출·견적 계산 전", "가격, 운영일, 회차 길이와 상품별 인원·할인 한도를 확정해야 합니다.", "승인값을 프로그램 설정과 검수 사례에 반영합니다.", "현재 화면의 5,000/4,000원·주말 값은 전 상품 공통값이 아닙니다. 예약 가능 일수는 ADM-02에서 프로그램별로 이미 설정 가능합니다."],
          ["D-02 시간 충돌 범위", "다른 지역 또는 연속 회차를 예약할 때", "지역 간 충돌 적용, 이동·준비시간과 종료=다음 시작 허용 여부를 확정해야 합니다.", "충돌 그룹과 시간 계산 기준에 반영합니다.", "현재 화면은 [시작, 종료) 기준으로 종료와 다음 시작이 같으면 허용합니다."],
          ...
          ["D-09 회원 · 개인정보", "소셜 계정 연결 또는 개인정보 처리", "네이버·카카오 계정 통합 여부와 수집 항목, 동의, 거래 이력 유지 기준이 필요합니다.", "회원 연결과 보존 정책에 반영합니다.", "현재는 제공자가 다르면 별도 계정이며 동일인 전체 제한과는 다릅니다. 회원탈퇴 시 삭제·재가입 처리는 RSV-08에서 확정했습니다."],
          ...
          ["D-14 오픈 범위 · 검수", "지역·상품·CTA를 공개할 때", "실제 오픈 대상, 순서, 연동 준비, QA·승인 일정과 지원 환경을 확정해야 합니다.", "단계별 공개와 완료 기준에 반영합니다.", "현재 두 상품 구성이나 과거 일정 의견을 전체 구축 완료로 표시하지 않습니다."]
        ]}
      ]
    }
  };
  ...
  var servicePolicy = {
    ...
    "RSV-04": [
      ...
      ["할인 적용", "할인 정책을 선택한 티켓", "온라인 증빙 절차 없이 활성 할인 정책의 대상·기간·일일 한도만 검사합니다.", "취소·환불이 완료된 할인 티켓 수만큼 해당 이용일의 할인 한도를 즉시 복원합니다.", "환불 확인 필요 상태에서는 환불 완료 전까지 할인 한도를 복원하지 않습니다."]
    ],
    "RSV-05": [
      ["장바구니 구성", "로그인 회원이 예약 옵션을 담을 때", "한 장바구니에는 동일한 이용일·지역·담당부서의 비중복 회차만 함께 담을 수 있습니다.", "다른 이용일·지역·담당부서 상품은 묶음 결제하지 않으며 기존 장바구니를 결제하거나 비운 뒤 담습니다.", "같은 상품·날짜·회차를 다시 담으면 별도 줄 대신 수량과 할인 선택을 갱신합니다."],
      ...
      ["장바구니 재고 점유", "담기 완료부터 결제 전까지", "장바구니에 담은 수량은 서버에서 10분간 점유하고 사용자에게 남은 시간을 표시합니다.", "수량 변경 시 10분을 다시 계산하며 삭제·결제·만료 시 점유를 즉시 해제합니다.", "만료 후에는 재고를 다시 확인해 담아야 하며 화면을 새로고침해도 서버 만료 시각을 기준으로 합니다."]
    ],
    "RSV-06": [
    ...
    "ADM-01": [["예약 검색 기준", "예약번호·결제번호·프로그램명 검색과 지역·부서·이용일·프로그램·상태 필터", "문자 검색은 앞뒤 공백을 제거한 부분 일치, 복수 필터는 AND 조건으로 적용합니다.", "검색 버튼과 Enter는 같은 요청을 실행하고 초기화는 기본 조건·첫 페이지로 재조회합니다.", "빈 결과와 오류를 구분하며 마지막 성공 결과를 오류 결과처럼 표시하지 않습니다."], ["예약 목록 페이지", "검색·필터가 적용된 예약 목록", "한 페이지에 20건을 최신 예약순으로 표시하고 하단에 현재 범위·전체 건수·최대 5개의 페이지 번호를 제공합니다.", "무한 스크롤이나 스크롤 고정 페이징은 사용하지 않습니다. 페이지 이동 시 목록 상단으로 스크롤하고 검색·필터·초기화 시 1페이지로 돌아갑니다.", "전체 선택은 현재 페이지 20건만 대상으로 하며 서버 API도 page·pageSize=20과 전체 건수를 반환해야 합니다."], ["예약 운영 권한", "예약 목록과 상세", "전체 프로그램 조회와 예약자·결제 정보 조회 권한을 구분합니다.", "허용된 지역·부서·기능 범위의 예약만 조회·처리합니다.", "다른 부서 프로그램 조회가 개인정보·취소 권한을 의미하지 않습니다."]],
    "ADM-02": [["프로그램 검색 · 초기화", "프로그램·회차 목록 상단 검색 영역", "프로그램명 필드만 부분 일치 검색하며 지역·부서 필터와 AND 결합합니다.", "검색은 버튼/Enter로 실행하고 새로고침은 조건을 유지한 최신 데이터 조회, 초기화는 모든 조건 제거 후 전체 조회입니다.", "페이지가 있다면 검색·초기화 시 1페이지로 이동합니다."], ["목록 축약 표기", "할인 정책·등록 회차처럼 복수 값인 열", "할인은 우선순위 첫 항목 뒤에 ‘외 N개’를 표시하고 상세/툴팁에서 전체 목록을 제공합니다.", "할인 0개는 ‘없음’, 1개는 이름만, 2개 이상은 ‘대표 할인 외 N개’로 표시합니다.", "등록 회차 수에는 숨김 회차도 포함하고 판매중 수를 보조 정보로 분리합니다."], ["프로그램 수명주기", "등록·수정·판매중·숨김", "신규 판매 상태와 과거 주문 스냅샷을 분리합니다.", "숨김은 신규 노출만 중단하고 기존 예약·결제·티켓은 보존합니다.", "저장 실패 시 기존 화면 상태를 유지하고 성공 응답으로 해당 행을 갱신합니다."]],
    "ADM-03": [["회차 운영", "반복 회차와 날짜별 실제 회차", "판매 수량은 현장 수량과 독립적으로 관리하며 회차는 시작 시각순으로 번호를 부여합니다.", "기존 예약이 있는 회차 변경은 영향 인원과 재고를 확인한 뒤 처리합니다.", "기존 예약 수보다 작은 수량 저장, 시간 중복과 종료≤시작은 차단합니다."], ["운영일 · 휴장일", "지역과 전체 또는 개별 프로그램의 단일 날짜·기간", "캘린더에는 프로그램에 이미 등록된 운영일을 표시하고 예약을 닫을 휴장일만 별도 저장합니다.", "전체 프로그램 휴장이 개별 프로그램 운영보다 우선하며 휴장 해제는 원 운영 일정에 포함된 날만 복원합니다.", "이미 예약된 날짜의 휴장 등록은 자동 환불이 아니며 별도 운영 취소 절차가 필요합니다."]],
    "ADM-04": [["할인 정책 · 복수 표시", "정률·정액 할인과 적용 프로그램", "할인은 프로그램과 분리하고 같은 프로그램에 여러 할인이 연결되면 목록에는 우선순위 1위+‘외 N개’로 표시합니다.", "MVP는 티켓당 할인 하나만 허용하며 서버 우선순위가 가장 높은 활성 할인 하나만 견적에 적용합니다.", "상세에는 전체 정책과 활성 상태를 표시하고 결제 당시 값은 주문 스냅샷으로 보존합니다."]],
  ...
  function topicsFor(context) {
    ...
    if (context === "admin") return withService([
      ...
      { key: "ADM-04", title: "할인 관리", summary: "복수 할인 축약 표기, 적용 우선순위와 과거 주문 보호", fe: [admin[5]], be: [adminApi[2]], pending: [pending[3], pending[4]] },

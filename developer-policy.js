(function () {
    ...
    frontend: {
      ...
      groups: [
        { title: "사용자 예약 화면", eyebrow: "CUSTOMER FLOW", items: [
          ...
          ["티켓 취소 · 입장", "티켓의 ‘예약 취소’ 또는 입장 상태를 확인", "본인 예약, 유효 인원, 프로그램별 취소 마감과 현재 취소·환불·입장 상태를 확인합니다.", "취소 가능 대상과 예상 환불액을 보여주고, 회차 시작 10분 전부터 종료 전까지는 ‘입장 가능’, 그 전은 ‘입장 대기’, 그 후는 ‘입장 종료’로 상태를 구분합니다. 과천시민 할인 인원 티켓에는 직원 확인용 증빙 확인 안내를 함께 표시합니다.", "사용자 직접 부분취소, 나눠 입장, 증빙 실패와 사용 처리 방식은 정책 확정 전 임의로 제공하지 않습니다."],
  ...
  var servicePolicy = {
    ...
    "RSV-07": [
      ...
      ["취소 · 입장", "사용자 취소 또는 현장 입장", "기본 취소 마감은 이용 시작 10분 전이며 취소 상태가 시간상 입장 가능 상태보다 우선합니다.", "취소 가능한 대상·예상 환불액과, 회차 시작 10분 전부터 종료 전까지 ‘입장 가능’·그 전 ‘입장 대기’·그 후 ‘입장 종료’ 상태를 구분합니다.", "사용자 직접 부분취소, 나눠 입장, 노쇼와 현장 증빙 실패는 D-03~D-05에서 확정합니다."],
  ...
  function topicsFor(context) {
    ...
    return withService([
      ...
      { key: "RSV-07", title: "내 티켓", summary: "회차별 티켓 표시, 상태 배지와 전체·부분취소·입장 처리", fe: [customer[9], customer[10], customer[11]], be: [bookingApi[5], bookingApi[7], bookingApi[8], bookingApi[10]], pending: [pending[2], pending[3], pending[4], pending[11]] },

(function () {
    ...
    frontend: {
      ...
      groups: [
        { title: "사용자 예약 화면", eyebrow: "CUSTOMER FLOW", items: [
          ...
          ["인원 · 할인", "인원 +/− 또는 할인 라디오 버튼을 클릭", "‘할인 미적용’과 현재 프로그램에 적용 가능한 활성 할인을 표시합니다. 최초 진입과 프로그램 변경 시 ‘할인 미적용’을 기본 선택하며, 정책이 여러 개여도 동일 이용일에는 하나만 선택할 수 있습니다.", "할인을 직접 선택한 경우에만 적용하고 수량·할인 선택에 따라 결제 예정 금액과 잔여 수량을 다시 계산합니다. 총 4명 중 할인 한도가 2명이면 2명은 할인, 나머지 2명은 정상가로 계산합니다.", "할인을 자동 적용하거나 가장 큰 할인으로 자동 변경하지 않습니다. 온라인에서 별도 할인 증빙을 수집하거나 검증하지 않습니다."],
          ...
          ["주문하기 · 결제", "‘N개 상품 주문하기’ 후 약관 동의 및 ‘결제하기’를 클릭", "결제 직전 최신 장바구니와 계정·재고·시간 충돌·할인 사용량을 다시 검사합니다. 약관 동의가 없으면 진행하지 않습니다.", "성공하면 결제 단위 예약번호 하나를 만들고 프로그램·회차별 티켓을 표시합니다.", "확인 화면 진입 후 장바구니가 바뀌면 약관 동의를 해제합니다. 처리 중 중복 클릭은 막습니다. 결제 완료 화면은 새로고침 시 유지되지 않으므로, 다시 접속하면 장바구니가 아니라 처음 화면으로 안내합니다."],
  ...
  var servicePolicy = {
    ...
    "ADM-07": [["조회 기준과 필터", "매출 · 정산 화면 전체", "조회 기간은 고객사가 요청한 실제 서비스 제공 완료일을 ‘서비스 이용일’로 표시해 사용합니다. 예약일과 결제일은 정산 대상 기간을 결정하지 않습니다.", "서비스 이용 시작일·종료일, 지역·부서, 카드사, 거래유형, 상품명 검색을 AND 조건으로 적용합니다. 위 요약 카드, 거래내역, 접힌 지역·부서·상품별 매출 요약과 엑셀 다운로드는 모두 같은 조회 결과를 사용합니다.", "시작일이 종료일보다 늦으면 조회 결과를 만들지 않고 오류를 표시하며 엑셀 다운로드를 비활성화합니다."], ["금액과 건수 산정", "조회 조건에 포함된 승인·취소 거래", "승인금액은 승인 거래의 합계, 취소금액은 전체·부분취소 거래 금액의 합계, 순매출은 승인금액에서 취소금액을 뺀 금액입니다.", "상단 카드는 승인금액·취소금액·수수료·지급예정액 순서로 표시합니다. 수수료는 거래별로 확인된 총수수료를 합산하되 취소에 따른 수수료 조정은 음수로 반영하고 ‘취소 수수료 조정 반영 완료’라고 안내합니다. 지급예정액은 순매출에서 총수수료를 뺀 금액이며 카드 아래에는 ‘순매출 - 수수료’라고 표시합니다.", "수수료 확인 대기·확인분·미확정 같은 별도 상태는 표시하지 않습니다. 지급예정액은 계산상 예정액이며 실제 입금 완료액을 뜻하지 않습니다."], ["거래내역과 매출 요약", "화면 조회 결과", "거래내역은 상품명, 서비스 이용일, 결제수단, 카드사, 구분, 거래금액, 수수료, 거래일시를 분리된 열로 표시합니다. 승인·부분취소·전체취소를 구분하고 취소 거래 금액은 음수로 표시합니다.", "행 상세에는 ‘imp_304455000252’ 형식의 포트원 거래번호, 지역·담당부서, 원결제일시, 취소사유, 지급예정액과 지급예정일을 표시합니다. 지역·부서·상품별 요약은 승인·취소 금액과 건수, 순매출, 수수료, 지급예정액을 집계하며 합계 행을 굵은 글씨와 상단 구분선으로 구분합니다.", "결제수단은 카드와 간편결제만 제공하고 계좌이체는 두지 않습니다. 예약번호·주문번호·PG 기술키·카드번호·승인번호·할부·세금·면세금액·수수료 공급가액과 부가세는 정산 화면에서 제외합니다."], ["엑셀 다운로드 내용", "현재 조회 조건의 전체 결과", "화면과 동일한 서비스 이용일·지역·부서·카드사·거래유형·상품명 조건과 동일한 계산 결과를 사용하며 현재 화면에 보이는 행 수와 무관하게 조건에 맞는 전체 거래를 포함합니다.", "‘매출 요약’ 시트에는 조회 정보, 승인금액·취소금액·순매출·수수료·지급예정액, 지역·부서·상품별 승인·취소 건수와 합계를 제공합니다. ‘개별 거래내역’ 시트에는 지역, 담당부서, 상품명, 서비스 이용일, 결제수단, 카드사, 거래구분, 승인금액, 취소금액, 수수료, 거래일시, 지급예정액, 지급예정일, 취소사유를 제공합니다.", "포트원 거래번호는 화면 상세에서만 확인하며 엑셀 열에는 넣지 않습니다. 결과가 0건이어도 조회 정보와 열 머리글이 있는 빈 엑셀을 생성합니다."], ["엑셀 표시 서식", "다운로드되는 두 시트", "모든 셀은 11pt와 기본 행 높이를 사용합니다. 표 머리글은 회색, 핵심 금액 머리글은 노란색, 합계 행은 연한 파란색으로 구분하며 매출 요약 합계의 순매출 셀은 노란색으로 강조합니다.", "테두리는 실제 표 머리글과 데이터·합계 범위에만 적용하고 조회 정보, 안내 문구와 구분용 빈 행에는 적용하지 않습니다. 숫자는 천 단위 구분과 음수 표시를 사용하며 개별 거래내역에는 틀 고정과 자동 필터를 제공합니다.", "행 높이나 글자 크기를 별도로 키우지 않고 줄무늬 배경을 사용하지 않습니다."], ["월 정산과 예외 환불", "서비스 이용일 기준 월 정산", "월 정산 대상은 해당 월에 실제 서비스가 완료된 건이며, 서비스 완료 전 부분취소를 함께 반영합니다. 미이용·서비스 전 전액취소 건은 제외합니다.", "포니체험은 결제 즉시 현장에서 완료되는 운영 특성을 반영합니다. 온라인 예약도 예약일이 아닌 실제 서비스 제공 완료일이 속한 월에 집계합니다.", "투어 프로그램의 정산 완료 후 환불 가능성은 브랜드총괄부 확인 사항입니다. 월 지급일과 예외 환불의 이전 달 소급 또는 다음 달 차감 기준은 확정 전까지 임의로 구현하지 않습니다."]],

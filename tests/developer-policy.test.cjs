...
test('each relevant screen loads the keyboard-only shared policy viewer', () => {
  for (const html of [storefront, admin, accountAdmin]) {
    ...
    assert.match(html, /developer-policy\.js\?v=20260907-devpolicy10/);
...
test('policy viewer joins service, frontend and backend rules with decisions', () => {
  assert.match(source, /type === "service"/);
  assert.match(source, /"적용 범위", "정책 기준", "확정 내용", "예외 · 미정"/);
  ...
  assert.match(source, /아직 확정이 필요한 정책/);
  ...
  assert.match(source, /D-14 오픈 범위 · 검수/);
  ...
  assert.match(source, /과천시민 할인 미체크/);
  assert.match(source, /과천시민 할인 체크/);

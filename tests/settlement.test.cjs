...
test('exact dates, cards, search and scope constrain screen/export source identically',()=>{
 ...
 assert.equal(L.ledgerHeaders.length,11);
 ...
 assert.equal(L.ledgerHeaders[0],'서비스 이용일');
 assert.equal(L.ledgerHeaders[1],'예약번호');
 ...
 assert.deepEqual(sheets.map(sheet=>sheet.name),['월 정산']);
 assert.deepEqual(sheets[0].rows[10],L.ledgerHeaders);
 assert.equal(sheets[0].rows.length,13);
...
test('Excel keeps monthly reconciliation and reservation details on one focused sheet',()=>{
 const rows=L.filter(may,()=>true),sheet=L.sheets(rows,may)[0],ledger=sheet.rows.slice(11,-1);
 assert.equal(ledger.length,L.groups(rows).length);
 const grouped=ledger.find(row=>row[1]==='LRP-260510-00003');
 assert.deepEqual(grouped,['2026-06-06','LRP-260510-00003','포니 타기','현대카드',15000,'부분 취소',10000,5000,100,4900,'2026-07-08']);
 assert.deepEqual(sheet.rows[7].slice(0,5),[73000,30000,43000,860,42140]);
 assert.equal(sheet.rows[7].length,5);
 assert.equal(sheet.rows.at(-1)[7],43000);
});
...
test('xlsx is a ZIP workbook with numeric amounts and literal formula-like strings',async()=>{
 const blob=SettlementXlsx.workbook([{name:'안전',rows:[['ID','금액'],['=1+1',27000],['00123',{formula:'IF(B2="","",B2*2)'}]]}]);
 ...
 assert.ok(!bytes.includes(Buffer.from('<f>1+1</f>')));
 assert.ok(bytes.includes(Buffer.from('<f>IF(B2=&quot;&quot;,&quot;&quot;,B2*2)</f>')));
 assert.ok(bytes.includes(Buffer.from('fullCalcOnLoad="1"')));
...
test('admin rendering and download use identical filtered ledger and reject reversed dates',async()=>{
 ...
 assert.match(elements['settlement-card-count'].textContent,/3건 결제 · 6개 거래/);
 ...
 assert.match(elements['settlement-detail-body'].innerHTML, /티켓별 내역<\/strong><small>카드 결제 · 2026-05-10T10:00:00<\/small>/);
 assert.ok(!elements['settlement-detail-body'].innerHTML.includes('최신 거래순'));
 assert.match(elements['settlement-detail-body'].innerHTML, /settlement-status is-cancelled">취소<\/span>/);
 assert.ok(!elements['settlement-detail-body'].innerHTML.includes('settlement-status is-cancelled">부분 취소'));
 ...
 assert.match(elements['settlement-metrics'].innerHTML,/<small>승인금액<\/small><strong>52000원<\/strong>/);
...
test('service month includes completed services and pre-service full cancellations',()=>{
 ...
 assert.equal(rows.length,12);assert.equal(t.approved,85000);assert.equal(t.cancelled,40000);assert.equal(t.net,45000);assert.equal(t.fee,900);assert.equal(t.payout,44100);assert.equal(t.net-t.fee,t.payout);
 assert.ok(['demo-pay-004','demo-pay-005','demo-pay-006'].every(id=>rows.some(e=>e.paymentId===id)));
 const fullyCancelled=L.groups(rows).filter(g=>L.reservationStatus(g)==='전체 취소');
 assert.equal(fullyCancelled.length,3);
 fullyCancelled.forEach(g=>assert.deepEqual(L.ledgerDetail(g).slice(7),[0,0,0,'']));
 assert.equal(L.filter({...f,start:'2026-05-01',end:'2026-05-31'},()=>true).length,0);
 assert.equal(L.filter({...f,asOf:'2026-06-06T14:59:59+09:00'},()=>true).length,6);
 assert.equal(L.sheets(rows,f)[0].rows[10][0],'서비스 이용일');
 assert.equal(L.sheets(rows,f)[0].rows[11][0],'2026-06-06');
});
...
test('date basis changes both filtered records and Excel period description',()=>{
 ...
 assert.equal(L.filter(service,()=>true).length,12);
 assert.match(L.sheets(L.filter(transaction,()=>true),transaction)[0].rows[4][1],/거래일 기준/);
 assert.match(L.sheets(L.filter(service,()=>true),service)[0].rows[4][1],/서비스 이용 완료일 기준/);
});

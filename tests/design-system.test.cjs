...
test('storefront uses design-system colors, radii and category tags', () => {
  ...
  assert.match(styles, /\.shop-steps\[hidden\]\{display:none\}/);
  assert.match(styles, /@media\(max-width:640px\)\{[\s\S]*?\.calendar-grid button\.is-available::before\{[^}]*right:7px;top:7px/);
  assert.match(styles, /\.calendar-grid button\.is-selected::after\{[^}]*right:-3px;top:-3px/);
  assert.match(styles, /\.discount-options \.discount-check:has\(input:checked\)\{border-color:var\(--ink\);box-shadow:none\}/);
  assert.match(html, /styles\.css\?v=20260921-image2x1/);
  assert.match(styles, /@media\(max-width:420px\)\{[\s\S]*?\.reservation-brand__context\{display:none\}[\s\S]*?\.shop-nav button\{padding-inline:4px\}/);
  assert.doesNotMatch(styles, /\.shop-nav__logout\{display:none\}/);
});

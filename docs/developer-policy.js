(function () {
    ...
    frontend: {
      ...
      groups: [
        { title: "사용자 예약 화면", eyebrow: "CUSTOMER FLOW", items: [
          ...
          ["인원 · 할인", "인원 +/− 또는 할인을 클릭", "일반 구매 수량에는 일일 한도를 두지 않고 회차 잔여 수량만 적용하며 할인은 정책별 일일 한도를 적용합니다.", "일반가·할인가·최종 결제 예정 금액을 즉시 다시 계산합니다.", "온라인 할인 증빙 기능은 제공하지 않습니다."],
          ["장바구니 담기", "날짜·회차·인원 선택 후 ‘장바구니 담기’를 클릭", "동일 이용일·지역·부서, 전 지역 시간 중복과 할인 사용량을 검사합니다.", "통과하면 재고를 10분간 점유하고 만료 시각과 함께 장바구니를 갱신합니다.", "수량 변경 시 10분을 다시 계산하며 삭제·결제·만료 시 점유를 해제합니다."],
          ...
          ["티켓 취소 · 이용 완료", "티켓의 ‘예약 취소’ 또는 시간 상태를 확인", "관리자 설정 취소 마감 전인 입장 대기 티켓만 취소할 수 있습니다.", "선택 티켓의 저장된 실제 결제액을 그대로 환불하고 회차 종료 시 이용 완료 처리합니다.", "입장 체크·노쇼·온라인 할인 증빙 기능은 제공하지 않습니다."]
        ]},
        { title: "운영 관리자 화면", eyebrow: "ADMIN FLOW", items: [
          ...
          ["할인 저장", "‘할인 관리’에서 추가/수정 후 저장을 클릭", "할인명, 정률 1~100% 또는 정액 1원 이상, 적용 기간, 최대 수량, 대상 프로그램을 검증하며 온라인 증빙 항목은 두지 않습니다.", "활성 할인만 신규 예약에 적용하고 환불 완료 시 취소 티켓 수만큼 이용일 한도를 복원합니다.", "변경 전 예약은 결제 당시 할인 스냅샷을 유지합니다."],
          ["선택 티켓 취소", "예약 상세에서 인원을 선택하고 ‘선택 취소·환불’을 클릭", "유효 티켓, 한 명 이상 선택, 취소 사유와 환불 권한을 확인합니다.", "예약 상태는 ‘예약 확정·취소 처리 중·부분 취소·취소 완료’로만 표시합니다.", "PG 응답 불명확·일부 실패는 예약 상태를 ‘취소 처리 중’으로 두고 상세 환불 내역에 ‘환불 확인 필요’를 별도로 표시합니다."],
        ...
        ,{ title: "위이 계정 관리 화면", eyebrow: "ACCOUNT ADMIN", items: [
          ...
          ["지역 계정 발급 · 재설정", "‘지역 계정 발급’ 또는 ‘계정 설정’을 클릭 후 저장", "서울·부산경남·제주에 각각 하나의 활성 지역 계정과 중복되지 않는 로그인 ID, 신규/재발급 임시 비밀번호를 검증합니다.", "기본 로그인 ID는 seoul_admin·busan_admin·jeju_admin이며 임시 비밀번호는 자동 생성해 최초 전달 후 다시 표시하지 않습니다.", "통합 관리자만 지역 계정을 발급·중지·재발급할 수 있습니다."],
          ["지역별 CRUD · 조회 권한", "지역 계정으로 프로그램·예약·환불·정산 기능 접근", "로그인 계정의 소속 지역과 대상 데이터의 지역을 비교합니다.", "자기 지역은 생성·조회·수정·삭제가 가능하고 다른 지역은 조회만 가능합니다.", "타 지역 C/U/D 요청은 화면 상태와 별개로 API에서도 403으로 거절합니다."],
    ...
    backend: {
      ...
      groups: [
        { title: "예약 · 결제 API", eyebrow: "BOOKING API", items: [
          ...
          ["장바구니 견적 · 재고 점유", "담기/수량 변경/주문 화면 진입 시 견적 요청", "동일 이용일·지역·부서, 전 지역 시간 중복, 할인 사용량과 재고를 서버 데이터로 검사합니다.", "재고 점유 토큰과 서버 기준 10분 만료 시각을 반환합니다.", "만료된 장바구니의 결제는 409로 거절하고 재선택을 안내합니다."],
          ...
          ["시간 충돌 판정", "견적과 결제 사전 검증 시", "같은 계정의 모든 지역·프로그램 유효 예약과 장바구니를 [시작, 종료) 구간으로 비교합니다.", "지역이 같거나 달라도 일부라도 겹치면 거절하고 종료와 다음 시작이 같으면 허용합니다.", "인원수와 주문 분할 여부에 관계없이 계정 단위로 검사합니다."],
          ...
          ["할인 사용량 복원", "취소·환불 완료 후", "취소 완료된 할인 티켓 수를 이용일별 할인 사용량에서 차감합니다.", "환불 성공과 같은 트랜잭션에서 일일 할인 한도를 즉시 복원합니다.", "환불 확인 필요 상태에서는 완료 전까지 복원하지 않습니다."],
          ...
          ["이용 완료 처리", "회차 종료 시각 경과", "별도 입장 확인 없이 취소되지 않은 예약의 회차 종료 여부만 확인합니다.", "회차가 끝나면 이용 완료로 자동 처리합니다.", "입장 여부·노쇼·알림 발송은 관리하지 않습니다."]
        ]},
        { title: "관리 · 운영 API", eyebrow: "ADMIN API", items: [
          ["관리자 조회 · 지역 권한", "모든 관리자 조회·저장·취소·다운로드 요청", "통합 관리자는 전체 지역 CRUD, 지역 관리자는 자기 지역 CRUD·타 지역 Read로 권한을 계산하고 대상 데이터의 지역을 확인합니다.", "허용 범위만 처리하고 계정 ID·소속 지역·대상 지역·작업·변경 전후 값을 감사 로그에 남깁니다.", "클라이언트의 지역 값을 신뢰하지 않으며 타 지역 C/U/D 요청은 403으로 거절합니다."],
          ...
          ["회차 일괄 취소", "운영 취소 확정 요청", "회차를 잠그고 유효 예약·결제, 환불 예정액, 중복 실행 여부를 조회합니다.", "예약별 환불 결과, 회차 중지, 티켓 취소와 감사 이력을 기록합니다.", "별도 알림은 발송하지 않으며 일부 환불 실패는 건별 재처리 대상으로 남깁니다."],
          ["정산 집계", "기간별 정산 조회 또는 파일 생성", "서비스 완료일 기준으로 지역·부서·프로그램별 집계하며 정산일은 매월 8일입니다.", "권한 범위의 결제액, 환불액, 지급 예정액과 근거 주문을 반환합니다.", "세금과 PG 수수료는 확정 전까지 임의 비율을 적용하지 않습니다."],
        ...
        { title: "아직 확정이 필요한 정책", eyebrow: "DECISION REQUIRED", pending: true, items: [
          ["D-01 상품 운영값", "상품 등록·예약 노출·견적 계산 전", "가격, 운영일, 예약 오픈 범위, 회차 길이와 상품별 인원·할인 한도를 확정해야 합니다.", "승인값을 프로그램 설정과 검수 사례에 반영합니다.", "현재 화면의 5,000/4,000원·주말·14일 값은 전 상품 공통값이 아닙니다."],
  ...
  function topicsFor(context) {
    ...
    if (context === "account") return [
      ...
      { key: "ACC-02", title: "계정 발급 · 재설정", summary: "지역별 운영 계정과 안전한 임시 비밀번호 처리", fe: [account[1]], be: [adminApi[5], adminApi[6]], pending: [pending[7], pending[12]] },
      { key: "ACC-03", title: "지역별 접근 권한", summary: "자기 지역 CRUD와 다른 지역 Read 전용 API 제어", fe: [account[2]], be: [adminApi[0], adminApi[6]], pending: [pending[7]] },
    ...
    return [
      ...
      { key: "RSV-07", title: "티켓 · 취소 · 입장", summary: "회차별 티켓 표시와 전체·부분취소·입장 처리", fe: [customer[10], customer[11]], be: [bookingApi[5], bookingApi[7], bookingApi[8], bookingApi[10]], pending: [pending[2], pending[3], pending[4], pending[11]] }
    ];
  }

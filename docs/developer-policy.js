(function () {
    ...
    frontend: {
      ...
      groups: [
        ...
        { title: "운영 관리자 화면", eyebrow: "ADMIN FLOW", items: [
          ...
          ["매출 · 정산 조회와 엑셀", "서비스 이용 시작일·종료일, 지역·부서, 카드사, 예약번호·상품명 검색 또는 ‘엑셀 다운로드 (.xlsx)’ 클릭", "시작일이 종료일보다 늦지 않은지 확인하고 모든 조회 조건을 AND로 적용합니다. 기간은 예약일·결제일이 아니라 실제 서비스 제공이 완료된 건의 서비스 이용일을 기준으로 하며, 계정에 허용된 지역·부서 데이터만 사용합니다. 화면의 요약 카드·결제·정산 원장·지역·부서·상품별 요약과 엑셀은 모두 같은 필터 결과로 계산합니다.", "상단 카드는 승인금액·취소금액·총수수료·지급예정액 순서로 표시합니다. 수수료 아래에는 ‘취소 수수료 조정 반영 완료’, 지급예정액 아래에는 ‘순매출 - 수수료’를 안내합니다. 지역·부서·상품별 요약의 합계 행은 상단 구분선과 굵은 글씨로 표시합니다. 엑셀은 현재 조회 조건의 ‘매출 요약’, 예약번호별 ‘결제·정산 원장’, 원본 ‘거래 이력’ 3개 시트로 내려받습니다.", "시작일이 종료일보다 늦으면 오류를 표시하고 다운로드를 비활성화합니다. 조회 결과가 없으면 두 표에 가운데 정렬된 빈 상태를 표시하며, 엑셀은 조회 정보와 각 시트의 열 머리글을 유지한 빈 양식으로 생성합니다. 현재 화면은 실제 API가 연결되지 않은 예시 데이터입니다."]
        ]}
    ...
    backend: {
      ...
      groups: [
        { title: "예약 · 결제 API", eyebrow: "BOOKING API", items: [
          ...
          ["동시간대 복수 티켓", "견적과 결제 사전 검증 시", "다른 프로그램의 이용 시간이 기존 유효 예약 또는 장바구니와 겹치면 제한합니다. 같은 프로그램의 같은 회차는 구매 한도와 재고 안에서 여러 매 구매할 수 있습니다.", "다른 프로그램의 이용 시간이 기존 유효 예약 또는 장바구니와 겹치면 제한합니다. 같은 프로그램의 같은 회차는 구매 한도와 재고 안에서 여러 매 구매할 수 있습니다.", "결제 직전 각 회차의 최신 재고와 계정별 이용일 한도를 다시 확인합니다."],
        ...
        { title: "관리 · 운영 API", eyebrow: "ADMIN API", items: [
          ...
          ["정산 집계 · 내보내기", "기간별 정산 조회 또는 XLSX 파일 생성", "예약·상품 데이터의 서비스 이용일·지역·부서·상품 정보와 포트원의 성공한 승인·취소 거래 정보를 서버에서 거래 단위로 연결합니다. 한국시간 서비스 제공 완료일과 계정 권한을 서버에서 검증하고 화면과 파일에 같은 필터와 산식을 적용합니다.", "승인과 취소를 별도 거래 이벤트로 보존해 승인금액, 취소금액, 순매출, 총수수료와 지급예정액을 반환합니다. PG 수수료는 거래금액의 2%로 계산하고 취소 거래에는 취소금액의 2%를 음수 조정합니다. 승인 거래의 지급예정일은 서비스 제공 완료월의 다음 달 8일로 계산합니다. 화면과 엑셀 원장에는 예약번호별 집계를 제공하고, 포트원 거래번호와 취소 티켓 번호는 거래 이력에서 원본 이벤트와 함께 제공합니다. 내부 주문번호와 PG 기술키는 제외합니다. 조회 결과가 0건이어도 세 시트의 조회 정보와 열 머리글을 반환합니다.", "수수료 공급가액·부가세는 운영 화면에서 나누어 보여주지 않습니다. 서비스 완료 후 예외 환불의 귀속 월은 확정 전까지 계산 규칙으로 추가하지 않습니다."],
  ...
  var servicePolicy = {
    ...
    "ADM-07": [["조회 기준과 필터", "매출 · 정산 화면 전체", "조회 기간은 고객사가 요청한 실제 서비스 제공 완료일을 ‘서비스 이용일’로 표시해 사용합니다. 예약일과 결제일은 정산 대상 기간을 결정하지 않습니다.", "서비스 이용 시작일·종료일, 지역·부서, 카드사, 예약번호·상품명 검색을 AND 조건으로 적용합니다. 위 요약 카드, 결제·정산 원장, 접힌 지역·부서·상품별 매출 요약과 엑셀 다운로드는 모두 같은 조회 결과를 사용합니다.", "시작일이 종료일보다 늦으면 조회 결과를 만들지 않고 오류를 표시하며 엑셀 다운로드를 비활성화합니다."], ["금액과 건수 산정", "조회 조건에 포함된 승인·취소 거래", "승인금액은 승인 거래의 합계, 취소금액은 전체·부분취소 거래 금액의 합계, 순매출은 승인금액에서 취소금액을 뺀 금액입니다.", "상단 카드는 승인금액·취소금액·수수료·지급예정액 순서로 표시합니다. 수수료는 거래별로 확인된 총수수료를 합산하되 취소에 따른 수수료 조정은 음수로 반영하고 ‘취소 수수료 조정 반영 완료’라고 안내합니다. 지급예정액은 순매출에서 총수수료를 뺀 금액이며 카드 아래에는 ‘순매출 - 수수료’라고 표시합니다.", "수수료 확인 대기·확인분·미확정 같은 별도 상태는 표시하지 않습니다. 지급예정액은 계산상 예정액이며 실제 입금 완료액을 뜻하지 않습니다."], ["결제·정산 원장과 매출 요약", "화면 조회 결과", "결제·정산 원장은 같은 예약번호의 승인·부분취소·전체취소를 한 행으로 묶고 예약번호·상품명, 서비스 이용일, 승인금액, 취소금액, 순매출, 수수료, 지급예정액과 지급예정일을 표시합니다. 제목 옆에는 결제 건수와 묶음 안의 원본 거래 건수를 함께 표시합니다.", "묶음 행을 펼치면 승인·취소 원본 거래를 최신순 타임라인으로 보존합니다. 각 이벤트에 거래일시, 구분, 거래금액, 결제수단, 카드사, 포트원 거래번호와 지역·담당부서를 표시하고, 취소 이벤트에는 취소된 인원별 티켓 번호와 취소사유를 함께 표시합니다. 이벤트별 수수료 문구는 표시하지 않고 묶음 행과 요약에서만 합산 결과를 제공합니다. 지역·부서·상품별 요약은 승인·취소 금액과 건수, 순매출, 수수료, 지급예정액을 집계하며 합계 행을 굵은 글씨와 상단 구분선으로 구분합니다.", "결제수단은 카드만 제공하며 간편결제와 계좌이체는 두지 않습니다. 주문번호·PG 기술키·카드번호·승인번호·할부·세금·면세금액·수수료 공급가액과 부가세는 정산 화면에서 제외합니다."], ["엑셀 다운로드 내용", "현재 조회 조건의 전체 결과", "화면과 동일한 서비스 이용일·지역·부서·카드사·예약번호·상품명 조건과 동일한 계산 결과를 사용하며 현재 화면에 보이는 묶음 행 수와 원본 거래 수를 모두 보존합니다.", "‘매출 요약’ 시트에는 조회 정보와 지역·부서·상품별 합계를, ‘결제·정산 원장’ 시트에는 예약번호별 승인금액·취소금액·순매출·수수료·지급예정액과 원본 거래 건수를, ‘거래 이력’ 시트에는 포트원 거래번호·거래일시·결제수단·카드사·거래구분·거래금액·취소 티켓 번호와 취소사유를 제공합니다.", "예약번호는 원장과 거래 이력을 연결하며, 포트원 거래번호와 취소 티켓 번호는 거래 이력에만 표시합니다. 결과가 0건이어도 조회 정보와 열 머리글이 있는 빈 엑셀을 생성합니다."], ["엑셀 표시 서식", "다운로드되는 세 시트", "모든 셀은 11pt와 기본 행 높이를 사용합니다. 표 머리글은 회색, 핵심 금액 머리글은 노란색, 합계 행은 연한 파란색으로 구분하며 매출 요약 합계의 순매출 셀은 노란색으로 강조합니다.", "테두리는 실제 표 머리글과 데이터·합계 범위에만 적용하고 조회 정보, 안내 문구와 구분용 빈 행에는 적용하지 않습니다. 숫자는 천 단위 구분과 음수 표시를 사용하며 결제·정산 원장과 거래 이력에는 틀 고정과 자동 필터를 제공합니다.", "행 높이나 글자 크기를 별도로 키우지 않고 줄무늬 배경을 사용하지 않습니다."], ["월 정산과 예외 환불", "서비스 이용일 기준 월 정산", "월 정산 대상은 해당 월에 실제 서비스가 완료된 건이며, 서비스 완료 전 부분취소를 함께 반영합니다. 미이용·서비스 전 전액취소 건은 제외합니다.", "포니체험은 결제 즉시 현장에서 완료되는 운영 특성을 반영합니다. 온라인 예약도 예약일이 아닌 실제 서비스 제공 완료일이 속한 월에 집계합니다.", "투어 프로그램의 정산 완료 후 환불 가능성은 브랜드총괄부 확인 사항입니다. 월 지급일과 예외 환불의 이전 달 소급 또는 다음 달 차감 기준은 확정 전까지 임의로 구현하지 않습니다."]],
  ...
  function topicsFor(context) {
    ...
    return withService([
      ...
      { key: "RSV-03", title: "회차 · 잔여 수량", summary: "동시간대 복수 티켓 허용과 잔여 좌석 판정", fe: [customer[3]], be: [bookingApi[1], bookingApi[6]] },

(function () {
  ...
  var servicePolicy = {
    ...
    "RSV-07": [
      ["예약(주문)번호 체계", "결제 완료 화면과 티켓에 주문번호를 표시할 때", "형식은 GP-YYMMDD-NNNNN 입니다. YYMMDD는 결제(주문) 발생일, NNNNN은 그날짜 안에서만 증가하는 5자리 순번입니다.", "날짜가 바뀌면 순번을 1부터 다시 시작하며, 같은 날짜 안에서는 순번이 항상 증가만 하므로 같은 번호가 두 번 나오지 않습니다. 사람이 눈으로 비교·구두 전달하기 쉽도록 UUID 대신 이 형식을 사용합니다.", "인원별 티켓 번호는 주문번호 뒤에 -T01, -T02처럼 순번만 붙여 구성하며 별도 채번 규칙을 두지 않습니다."],
    ...
    "ADM-07": [["예약(주문)번호 체계", "거래 원장 화면과 CSV에 예약번호를 표시할 때", "형식은 GP-YYMMDD-NNNNN 입니다. YYMMDD는 결제(주문) 발생일, NNNNN은 그날짜 안에서만 증가하는 5자리 순번입니다.", "날짜가 바뀌면 순번을 1부터 다시 시작하며, 같은 날짜 안에서는 순번이 항상 증가만 하므로 같은 번호가 두 번 나오지 않습니다. 사람이 눈으로 비교·구두 전달하기 쉽도록 UUID 대신 이 형식을 사용합니다.", "인원별 티켓 번호는 주문번호 뒤에 -T01, -T02처럼 순번만 붙여 구성하며 별도 채번 규칙을 두지 않습니다."], ["정산 조회 · 초기화", "기간·지역·부서 필터", "서비스 완료일 기준으로 집계하며 월 정산일은 매월 8일입니다.", "초기화는 기본 기간과 전체 지역·부서로 재조회하며 화면과 다운로드는 같은 필터·산식을 사용합니다.", "세금·PG 수수료가 확정되기 전에는 임의 비율을 적용하지 않습니다."], ["거래 원장 · 다운로드", "지역·부서·프로그램별 서비스 완료 건", "결제일이 아닌 서비스 제공 완료일 기준으로 결제액·환불액·지급 예정액을 구분합니다.", "상세는 예약·결제 1건 단위 원장을 제공하고 파일에는 조회 조건·생성 시각을 포함합니다.", "대용량 파일은 비동기 생성하며 다운로드 이력을 기록합니다."]],

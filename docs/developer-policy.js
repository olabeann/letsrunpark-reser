(function () {
    ...
    frontend: {
      ...
      groups: [
        ...
        ,{ title: "위이 계정 관리 화면", eyebrow: "ACCOUNT ADMIN", items: [
          ...
          ["계정 삭제 · 중지 · 비밀번호 재발급", "계정 삭제, 사용 중지 또는 새 임시 비밀번호 저장", "통합 관리자는 삭제할 수 없습니다. 부서 계정 삭제 시 로그인·업무 처리 이력과 프로그램·회차·예약·결제·환불·정산 연결 데이터가 전혀 없는지 확인합니다.", "미사용 계정만 확인 후 완전 삭제합니다. 이력이 있으면 삭제를 막고 사용 중지로 로그인과 기존 세션만 차단하며 업무 데이터는 보존합니다.", "서버가 삭제 직전 조건을 다시 검사하고 삭제 요청자·시각·대상·사유·결과를 대상 계정과 분리된 감사 로그에 남깁니다."]
        ]}
      ]
    },
    backend: {
      ...
      groups: [
        ...
        { title: "관리 · 운영 API", eyebrow: "ADMIN API", items: [
          ...
          ["계정 수명주기 · 감사", "계정 발급, 비밀번호 재발급, 사용 중지 또는 삭제", "통합 관리자 권한과 부서당 공용 계정 1개 제한을 검증합니다. 통합 관리자는 삭제 불가이며 부서 계정은 로그인·업무 이력과 프로그램·회차·예약·결제·환불·정산 연결 데이터가 없을 때만 삭제합니다.", "삭제 조건을 트랜잭션 안에서 재검사합니다. 이력이 있으면 사용 중지와 기존 세션 만료만 적용하고 업무 데이터는 보존하며 요청자·대상·사유·결과를 계정과 분리된 감사 로그에 기록합니다.", "계정과 업무 데이터는 연쇄 삭제하지 않습니다. 부서 공용 계정은 개인 처리자를 구분하지 못하므로 중요한 작업에는 처리자 메모를 함께 남깁니다."],
  ...
  var servicePolicy = {
    ...
    "ACC-01": [["계정 관리 접근", "위이 통합 관리자", "부서 계정 발급과 다른 계정의 상태·권한 조회는 내부 운영 기능입니다.", "부서 계정과 고객 예약 화면에는 접근 경로를 제공하지 않습니다.", "통합 관리자만 계정 발급·중지·재발급·미사용 계정 삭제를 수행합니다."]],
    ...
    "ACC-04": [["계정 삭제 · 중지 · 감사", "삭제, 사용 중지, 권한 변경, 비밀번호 재발급", "통합 관리자는 삭제할 수 없고 미사용 부서 계정만 완전 삭제합니다. 프로그램 또는 로그인·업무 처리 이력이 있으면 삭제를 막고 사용 중지로 전환합니다.", "삭제 직전 서버가 연결 이력을 다시 검사하며 계정 삭제가 프로그램·예약·결제 등의 연쇄 삭제로 이어지지 않게 합니다.", "삭제·중지 요청자와 시각, 대상, 사유, 결과를 별도 감사 로그에 기록합니다."]]
  };
  ...
  function topicsFor(context) {
    ...
    if (context === "account") return withService([
      ...
      { key: "ACC-04", title: "삭제 · 중지 · 보안", summary: "미사용 계정 삭제 제한, 세션 만료와 감사 이력", fe: [account[3]], be: [adminApi[5], adminApi[6], adminApi[7]] }
    ]);

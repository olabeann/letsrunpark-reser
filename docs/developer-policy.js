(function () {
    ...
    frontend: {
      ...
      groups: [
        { title: "사용자 예약 화면", eyebrow: "CUSTOMER FLOW", items: [
          ...
          ["회차 선택", "이용 시간을 클릭", "회차 판매 상태와 온라인 잔여 수량, 이용일 하루 구매 한도를 확인합니다.", "다른 상품이나 기존 예약과 이용 시간이 겹치더라도 여러 이용자의 티켓을 함께 구매할 수 있으므로 잔여 좌석이 있으면 회차를 선택할 수 있습니다.", "판매 수량은 회차 등록 시점에 1명 이상만 저장되며, 값이 비정상적으로 0 이하로 들어오더라도 임의의 기본 인원수로 대체해 예약을 허용하지 않습니다."],
          ...
          ["장바구니 담기", "날짜·회차·인원 선택 후 ‘장바구니 담기’를 클릭", "로그인, 필수 선택값, 동일 이용일·지역·부서, 회차 잔여 수량과 할인 사용량을 검사합니다.", "재고를 미리 점유하지 않고 통과하면 선택 인원수만큼 1명 단위 장바구니 카드를 만듭니다. 같은 상품·날짜·회차가 이미 담겨 있거나 다른 상품과 시간이 겹쳐도 합치지 않으며 카드별 수량 변경 없이 개별 삭제만 제공합니다.", "모든 성공 건은 ‘장바구니에 담았습니다.’라고 안내합니다. 동일 회차 카드 수를 합산해 일일 구매 한도와 잔여 수량을 검사하며 실제 잔여 수량은 결제 시점에 다시 확인합니다."],
    ...
    backend: {
      ...
      groups: [
        { title: "예약 · 결제 API", eyebrow: "BOOKING API", items: [
          ...
          ["동시간대 복수 티켓", "견적과 결제 사전 검증 시", "한 계정이 여러 이용자의 티켓을 함께 구매할 수 있으므로 기존 예약·장바구니와 시간대가 겹치는지만으로 차단하지 않습니다.", "같은 회차 또는 서로 다른 상품의 겹치는 회차도 온라인 잔여 수량과 이용일 하루 구매 한도 안에서 허용합니다.", "결제 직전 각 회차의 최신 재고와 계정별 이용일 한도를 다시 확인합니다."],
  ...
  var servicePolicy = {
    ...
    "RSV-03": [
      ["동시간대 예약", "같은 계정의 확정 예약과 장바구니 전체", "한 계정이 여러 이용자의 티켓을 함께 구매할 수 있으므로 다른 상품이나 기존 예약과 시간이 겹쳐도 회차를 선택하고 장바구니에 담을 수 있습니다.", "선택 가능 여부는 회차 판매 상태, 온라인 잔여 수량, 이용일 하루 구매 한도와 할인 한도로 판정합니다.", "같은 회차를 다시 담아도 기존 카드와 합치지 않고 인원마다 1명 단위 카드로 표시합니다."]
    ],
    "RSV-04": [
      ["일반 구매 수량", "포니 타기·포니랑 놀기 예약", "같은 계정은 같은 이용일의 두 포니 프로그램을 합쳐 최대 4매까지 예약할 수 있습니다.", "장바구니와 기존 유효 예약을 합산하고 결제 단계에서 최신 잔여 수량과 구매 한도를 다시 검사합니다.", "다른 이용일에는 한도가 새로 시작되며 관리자가 설정한 회차 판매 수량도 초과할 수 없습니다."],

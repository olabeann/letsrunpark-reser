(function () {
    ...
    frontend: {
      ...
      groups: [
        { title: "사용자 예약 화면", eyebrow: "CUSTOMER FLOW", items: [
          ...
          ["회차 선택", "이용 시간을 클릭", "회차 판매 상태와 온라인 잔여 수량, 이용일 하루 구매 한도를 확인합니다.", "다른 프로그램의 이용 시간이 기존 유효 예약 또는 장바구니와 겹치면 제한합니다. 같은 프로그램의 같은 회차는 구매 한도와 재고 안에서 여러 매 구매할 수 있습니다.", "판매 수량은 회차 등록 시점에 1명 이상만 저장되며, 값이 비정상적으로 0 이하로 들어오더라도 임의의 기본 인원수로 대체해 예약을 허용하지 않습니다."],
          ["인원 · 할인", "인원 +/−, 할인 라디오 또는 예약 버튼을 클릭", "이용 날짜와 이용 시간을 순서대로 선택했는지 먼저 확인합니다. 날짜가 없으면 날짜 선택, 날짜만 있으면 시간 선택을 요청하는 토스트를 표시하고 인원·할인을 변경하지 않습니다.", "인원은 항상 1명에서 시작하며 할인을 선택해도 자동으로 늘리지 않습니다. 할인은 남은 할인 가능 매수까지, 할인 미적용은 구매 한도와 회차 잔여 수량 안에서 최대 4매까지 직접 조절할 수 있습니다.", "수량을 더 변경할 수 없으면 장바구니·기존 예약·회차 잔여석 중 제한 원인과 추가 가능 매수를 토스트로 표시합니다. 담기는 한 번에 처리하되 장바구니에는 1명 단위 카드로 나누어 표시합니다."],
          ["장바구니 담기", "날짜·회차·인원 선택 후 ‘장바구니 담기’를 클릭", "로그인, 필수 선택값, 동일 이용일·지역·부서, 회차 잔여 수량과 할인 사용량을 검사합니다.", "재고를 미리 점유하지 않고 통과하면 선택 인원수만큼 1명 단위 장바구니 카드를 만듭니다. 다른 프로그램의 이용 시간이 기존 유효 예약 또는 장바구니와 겹치면 제한합니다. 같은 프로그램의 같은 회차는 구매 한도와 재고 안에서 여러 매 구매할 수 있습니다.", "모든 성공 건은 ‘장바구니에 담았습니다.’라고 안내합니다. 동일 회차 카드 수를 합산해 일일 구매 한도와 잔여 수량을 검사하며 실제 잔여 수량은 결제 시점에 다시 확인합니다."],
          ["바로 예약", "‘예약하기’를 클릭", "장바구니 담기와 같은 조건을 검사하고 선택 상품을 장바구니에 반영합니다. 반영 직전 현재 계정의 장바구니가 비어 있는지도 확인합니다.", "기존 장바구니가 비어 있으면 예약 내용 확인 화면으로 바로 이동합니다. 기존 상품이 1개 이상이면 선택 상품을 추가한 뒤 장바구니 화면으로 이동해 전체 상품을 확인하고 주문하도록 합니다.", "다른 이용일 상품이 있거나 구매·할인·재고 한도를 넘으면 새 상품을 추가하지 않습니다. 기존 상품 때문에 일일 구매 한도를 넘으면 장바구니로 이동하되, 선택 상품명·인원과 ‘담기지 않았습니다. 기존 상품 확인 후 결제해주세요.’를 토스트와 장바구니 오류 영역에 명확히 표시합니다."],
          ...
          ["장바구니 수정", "장바구니 카드의 ‘삭제’를 클릭", "삭제 뒤에도 단일 이용일·구매·재고·할인 한도를 다시 확인합니다.", "선택한 1명 카드를 제거하고 상품 금액, 할인 금액, 총액과 주문 버튼 활성 상태를 갱신합니다.", "장바구니 카드의 인원수는 1명으로 고정하며 +/− 수량 변경 기능은 제공하지 않습니다."],
          ["주문하기 · 결제", "‘N개 티켓 주문하기’ 후 약관 동의 및 ‘결제하기’를 클릭", "결제 직전 최신 장바구니와 계정·재고·구매 한도·할인 사용량을 다시 검사합니다. 약관 동의가 없으면 진행하지 않습니다.", "성공하면 결제 단위 예약번호 하나를 만들고 프로그램·회차별 티켓을 표시합니다.", "확인 화면 진입 후 장바구니가 바뀌면 약관 동의를 해제합니다. 처리 중 중복 클릭은 막습니다. 결제 완료 화면은 새로고침 시 유지되지 않으므로, 다시 접속하면 장바구니가 아니라 처음 화면으로 안내합니다."],
          ["예약 티켓", "‘예약 티켓 보기’ 또는 예약 목록의 티켓을 클릭", "로그인 계정의 유효 예약만 조회하고 이용 시각 기준으로 예정·입장 가능·종료 상태를 계산합니다.", "같은 프로그램·같은 회차의 복수 인원은 한 티켓에 인원수로 표시하고, 카드에는 입장 대기·입장 가능·입장 종료 상태 배지와 실제 결제 때 적용된 할인명을 함께 보여줍니다. 할인 미적용 티켓에는 할인 문구를 표시하지 않습니다.", "다른 프로그램 또는 다른 회차는 별도 티켓이며 취소된 티켓은 유효 인원에서 제외합니다."],
          ["티켓 상태 · 정보 표시", "티켓 목록 카드와 상세 화면 진입", "현재 시각, 예약 시 저장한 프로그램별 입장 대기 시간(기본 20분), 회차 시작·종료 시각을 비교해 상태를 자동 계산하며 별도 입장 체크 기능은 사용하지 않습니다.", "대기 시간부터 회차 종료 전까지 티켓을 활성화하고 ‘N분 전까지 방문하셔서 입장을 대기해주세요.’라고 표시합니다. 카드에는 입장 대기·입장 가능·입장 종료 상태와 프로그램, 이용일·회차, 인원수, 결제금액, 할인 여부를 표시합니다. 회차 종료 시 운영상 ‘이용 완료’로 자동 처리합니다.", "입장 종료·이용 완료는 실제 입장 여부나 노쇼를 의미하지 않으며 노쇼 상태와 수동 입장 처리는 제공하지 않습니다."],
          ...
          ["회원탈퇴", "‘예약 티켓’ 화면의 ‘회원탈퇴’를 클릭", "시스템 확인 모달로 탈퇴 의사를 다시 확인합니다.", "이름·연락처 등의 회원정보는 수집하지 않으며 탈퇴 시 소셜 로그인 연결과 장바구니를 제거합니다.", "결제·예약 이력은 운영 기록으로 계속 보관하고 재가입은 제한하지 않습니다."],
        ...
        { title: "운영 관리자 화면", eyebrow: "ADMIN FLOW", items: [
          ...
          ["매출 · 정산 조회와 엑셀", "서비스 이용 시작일·종료일, 지역·부서, 카드사, 예약번호·상품명 검색 또는 ‘엑셀 다운로드 (.xlsx)’ 클릭", "시작일이 종료일보다 늦지 않은지 확인하고 모든 조회 조건을 AND로 적용합니다. 기간은 예약일·결제일이 아니라 실제 서비스 제공이 완료된 건의 서비스 이용일을 기준으로 하며, 계정에 허용된 지역·부서 데이터만 사용합니다. 화면의 요약 카드·결제·정산 원장·지역·부서·상품별 요약과 엑셀은 모두 같은 필터 결과로 계산합니다.", "상단 카드는 승인금액·취소금액·총수수료·지급예정액 순서로 표시합니다. 엑셀은 현재 조회 조건의 ‘월 정산’ 한 시트로 내려받으며, 상단 금액 요약 아래에 예약번호별 취소 여부·취소 제외 매출·수수료·입금예정액을 제공합니다.", "시작일이 종료일보다 늦으면 오류를 표시하고 다운로드를 비활성화합니다. 조회 결과가 없어도 조회 정보와 열 머리글이 있는 빈 양식을 생성합니다. 현재 화면은 실제 API가 연결되지 않은 예시 데이터입니다."]
        ]}
        ,{ title: "운영 관리자 계정·권한 화면", eyebrow: "ACCOUNT ADMIN", items: [
          ["계정 검색 · 조회", "지역 필터 또는 계정 검색어를 변경", "통합 관리자 권한으로 지역·부서·로그인 ID 조건을 적용합니다.", "조건에 맞는 계정과 권한 범위를 표시합니다.", "부서 계정에는 계정 발급 화면과 다른 계정의 보안 정보를 노출하지 않습니다."],
          ["부서 계정 발급 · 재설정", "‘부서 계정 발급’ 또는 ‘계정 정보’를 클릭 후 저장", "같은 지역·부서에 공용 계정이 중복되지 않는지와 로그인 ID, 신규/재발급 임시 비밀번호를 검증합니다.", "지역과 담당 부서를 선택해 공용 계정을 발급하고 임시 비밀번호는 최초 전달 후 다시 표시하지 않습니다.", "통합 관리자만 부서 계정을 발급·삭제·재발급할 수 있으며 부서 계정은 자기 소속 범위를 변경할 수 없습니다."],
          ...
          ["계정 삭제 · 비밀번호 재발급", "계정 삭제 또는 새 임시 비밀번호 저장", "통합 관리자는 삭제할 수 없습니다. 부서 계정 삭제 시 프로그램·회차·예약·결제·환불·정산 등 운영 이력이 전혀 없는지 확인합니다.", "연결된 프로그램이나 운영 이력이 없을 때만 확인 후 삭제합니다. 이력이 있으면 삭제를 차단하고 사유를 안내합니다. 로그인 이력만으로는 삭제를 제한하지 않습니다.", "서버가 삭제 직전 조건을 다시 검사하고 삭제 요청자·시각·대상·사유·결과를 대상 계정과 분리된 감사 로그에 남깁니다."]
        ]}
      ]
    },
    backend: {
      ...
      groups: [
        { title: "예약 · 결제 API", eyebrow: "BOOKING API", items: [
          ...
          ["장바구니 견적", "담기/수량 변경/주문 화면 진입 시 견적 요청", "상품·회차 ID, 수량, 동일 이용일·지역·부서, 회차 잔여 수량, 활성 할인과 사용량을 서버 데이터로 계산합니다.", "재고를 점유하지 않고 요청 시점의 견적 금액과 잔여 수량만 반환합니다.", "실제 재고 확정은 결제 사전 검증에서 이루어지며, 여러 장바구니가 같은 회차를 담아도 이 단계에서는 거절하지 않습니다."],
          ["결제 사전 검증", "결제하기 클릭 후 주문 생성 요청", "최신 회차 상태, 온라인 재고, 기존 확정 예약+새 주문 수량, 구매 한도와 할인 사용량을 잠금 상태에서 재검사합니다.", "통과 건만 결제 준비 주문을 만들고 PG 요청 금액을 확정합니다.", "동시 요청으로 재고가 바뀌면 409와 최신 잔여 수량을 반환합니다."],
          ...
          ["주문 스냅샷 · 티켓", "주문 확정 시", "프로그램명, 지역·부서, 이용일·회차, 정가, 할인 정책/금액, 결제액과 입장 대기 시간을 주문 항목에 복사합니다.", "이후 프로그램·회차·할인 또는 대기 시간이 수정되어도 과거 주문과 정산은 유지됩니다.", "표시는 회차별로 묶되 부분취소를 위해 인원별 티켓과 금액 배분값을 보관합니다."],
          ["프로그램 간 시간 중복", "견적과 결제 사전 검증 시", "다른 프로그램의 이용 시간이 기존 유효 예약 또는 장바구니와 겹치면 제한합니다. 같은 프로그램의 같은 회차는 구매 한도와 재고 안에서 여러 매 구매할 수 있습니다.", "취소 완료 티켓은 제외합니다. 종료 시각과 다음 시작 시각이 같으면 허용하고 이동·준비시간은 추가하지 않습니다.", "결제 직전 각 회차의 최신 재고와 계정별 이용일 한도를 다시 확인합니다."],
        ...
        { title: "관리 · 운영 API", eyebrow: "ADMIN API", items: [
          ...
          ["정산 집계 · 내보내기", "기간별 정산 조회 또는 XLSX 파일 생성", "예약·상품 데이터의 서비스 이용일·지역·부서·상품 정보와 포트원의 성공한 승인·취소 거래 정보를 서버에서 거래 단위로 연결합니다. 한국시간 서비스 제공 완료일과 계정 권한을 서버에서 검증하고 화면과 파일에 같은 필터와 산식을 적용합니다.", "승인과 취소를 별도 거래 이벤트로 보존해 승인금액, 취소금액, 취소 제외 매출, 총수수료와 입금예정액을 계산합니다. PG 수수료는 거래금액의 2%로 계산하고 취소 거래에는 취소금액의 2%를 음수 조정합니다. 엑셀은 월 정산 한 시트에 요약·예약번호별 거래내역을 제공하며 원본 기술 식별자는 제외합니다. 조회 결과가 0건이어도 조회 정보와 열 머리글을 반환합니다.", "수수료 공급가액·부가세는 운영 화면에서 나누어 보여주지 않습니다. 서비스 완료 후 예외 환불의 귀속 월은 확정 전까지 계산 규칙으로 추가하지 않습니다."],
  ...
  var servicePolicy = {
    ...
    "RSV-03": [
      ["동시간대 예약", "같은 계정의 확정 예약과 장바구니 전체", "다른 프로그램의 이용 시간이 기존 유효 예약 또는 장바구니와 겹치면 제한합니다. 같은 프로그램의 같은 회차는 구매 한도와 재고 안에서 여러 매 구매할 수 있습니다.", "선택 가능 여부는 회차 판매 상태, 온라인 잔여 수량, 이용일 하루 구매 한도와 할인 한도로 판정합니다.", "같은 회차를 다시 담아도 기존 카드와 합치지 않고 인원마다 1명 단위 카드로 표시합니다."]
    ],
    "RSV-04": [
      ["일반 구매 수량", "포니 타기·포니랑 놀기 예약", "프로그램마다 계정당 이용일 최대 구매 수량을 설정합니다. 기본값은 4매이며 다른 프로그램 수량은 합산하지 않습니다.", "장바구니와 기존 유효 예약을 합산하고 결제 단계에서 최신 잔여 수량과 구매 한도를 다시 검사합니다.", "다른 이용일에는 한도가 새로 시작되며 관리자가 설정한 회차 판매 수량도 초과할 수 없습니다."],
      ["할인 적용", "할인 정책을 선택한 티켓", "온라인 증빙 절차 없이 활성 할인 정책의 대상·기간·일일 한도를 검사하고 할인 선택 인원마다 1명 단위 할인 카드를 생성합니다.", "인원은 1명에서 시작하고 해당 이용일의 남은 할인 매수까지만 직접 늘릴 수 있습니다. 한도를 모두 사용하면 할인 선택을 비활성화하며, 정상가 인원은 ‘할인 미적용’으로 따로 담습니다.", "모든 장바구니 카드는 1명으로 고정하고 할인 적용 여부를 혼합하지 않습니다. 취소 요청이 실패하면 티켓과 할인 사용량을 모두 기존 상태로 유지합니다."]
    ],
    "RSV-05": [
      ["장바구니 구성", "로그인 회원이 예약 옵션을 담을 때", "한 장바구니에는 동일한 이용일·지역·담당부서의 상품만 함께 담을 수 있습니다. 같은 상품·날짜·회차의 반복 담기도 허용합니다.", "다른 이용일·지역·담당부서 상품은 묶음 결제하지 않으며 기존 장바구니를 결제하거나 비운 뒤 담습니다.", "선택 인원수만큼 1명 단위 카드를 만들고 기존 카드와 합치지 않습니다. 장바구니에서는 인원을 증감하지 않고 카드별 삭제만 제공하며, 동일 회차 카드 수를 합산해 회차 정원·일일 구매 한도·할인 한도를 검사합니다."],
    ...
    "RSV-06": [
      ["예약하기 진입 경로", "예약 화면에서 ‘예약하기’를 선택할 때", "선택 상품은 결제 전 장바구니 데이터에 반영하되, 클릭 직전 장바구니 상품 유무에 따라 다음 화면을 결정합니다.", "장바구니가 비어 있었다면 예약 내용 확인 화면으로 바로 이동하고, 기존 상품이 있었다면 새 상품까지 담은 장바구니 화면에서 전체 주문을 먼저 확인합니다.", "선택 상품이 장바구니 정책·구매 한도·할인 한도·재고 검증을 통과하지 못하면 결제 화면으로 이동하지 않습니다. 기존 장바구니가 구매 한도를 채운 경우 선택 상품은 저장하지 않고, 상품명·인원과 미반영 사실을 장바구니 화면에 지속 표시합니다."],
      ["예약 · 결제 확정", "장바구니 결제 또는 바로 예약", "한 번의 결제에는 예약을 하나만 만들고 선택한 상품·이용일·회차는 그 예약 아래 티켓으로 관리합니다.", "PG 승인 검증과 서버 저장이 모두 끝난 뒤에만 예약 완료와 티켓을 표시합니다.", "결제 버튼 클릭이나 완료 화면 노출만으로 성공으로 판단하지 않습니다."],
      ["예약번호 체계 · 개발 검토 반영", "결제 확정 시 예약번호를 발급하고 예약 목록·상세와 통합예약목록 CSV에 표시할 때", "하나의 예약 아래 여러 티켓을 관리할 수 있다는 개발 의견을 반영해, 기존 예약번호의 상품 구분용 -1, -2 접미사만 제거합니다. 형식은 LRP-YYMMDD-NNNNN으로 유지합니다.", "예: ‘포니 타기’와 ‘포니랑 놀기’를 함께 결제하면 기존 LRP-260909-00001-1·LRP-260909-00001-2 대신 두 티켓 모두 공통 예약번호 LRP-260909-00001을 표시합니다.", "상품·회차는 티켓 정보로 구분하며 인원별 티켓 번호(-T01, -T02 등)와 PG 거래번호는 예약번호와 별도로 관리합니다."],
      ["결제번호 · 포트원 거래번호 구분", "결제 승인·영수증 조회·취소·환불 내역을 저장하고 표시할 때", "PAY-YYMMDD-NNNN 형식의 결제번호는 서비스가 발급하는 내부 식별자로, 한 번의 결제에 속한 예약·티켓과 승인·환불 누계를 연결해 관리합니다. 화면에서는 ‘통합 결제번호’가 아닌 ‘결제번호’로 표시합니다.", "imp_로 시작하는 포트원 거래번호는 포트원이 발급하는 외부 결제 식별자로, 포트원 결제 조회·검증·취소와 장애 문의에 사용합니다.", "두 번호는 같은 결제 건에 연결해 함께 저장하되 서로 대체하지 않습니다. 결제번호는 사용자 결제 내역·영수증과 관리자 결제 상세에 표시하고, 포트원 거래번호는 관리자 결제 상세에서만 표시합니다."],
    ...
    "RSV-07": [
      ["예약 티켓 목록 정렬", "예약 티켓 목록 진입 또는 재조회", "같은 예약번호의 티켓을 하나의 묶음으로 유지하며 기존 예약 묶음 순서는 유지합니다.", "각 예약 묶음 안에서 입장 가능 → 입장 대기 → 입장 종료 순으로 티켓을 표시합니다. 같은 상태의 티켓은 기존 순서를 유지하며 날짜·회차 시간으로 추가 정렬하지 않습니다.", "목록 진입·재조회 시 정렬을 적용합니다. 화면을 보고 있는 동안에는 상태 배지만 자동 갱신하며 티켓 위치는 유지합니다."],
      ["예약번호 체계 · 개발 검토 반영", "결제 완료 화면과 티켓에 예약번호를 표시할 때", "하나의 예약 아래 여러 티켓을 관리할 수 있으므로 기존 상품별 예약번호의 -1, -2만 제거합니다. 한 번의 결제에 LRP-YYMMDD-NNNNN 형식의 예약번호 하나를 표시합니다.", "예: ‘포니 타기’와 ‘포니랑 놀기’를 함께 결제하면 두 사용자 티켓 모두 LRP-260909-00001을 표시합니다.", "상품·회차는 티켓에서 구분하며 인원별 티켓과 PG 거래는 별도 식별자로 관리합니다."],
      ["티켓 표시 단위", "결제 완료된 예약 조회", "사용자는 상품·이용일·회차별 묶음 티켓 한 장에서 유효 인원을 확인합니다. 할인 적용 수량이 있으면 목록과 상세에 결제 당시 할인명을 표시하고, 할인 미적용 티켓에는 할인 문구를 표시하지 않습니다.", "관리자는 예약 인원별 개별 티켓에서 프로그램·회차·결제 당시 할인명·결제액·상태와 취소·환불 이력을 관리합니다.", "취소 완료 인원은 묶음 티켓의 유효 인원에서 제외합니다."],
    ...
    "ADM-07": [["조회 기준과 필터", "매출 · 정산 화면 전체", "조회 기간은 고객사가 요청한 실제 서비스 제공 완료일을 ‘서비스 이용일’로 표시해 사용합니다. 예약일과 결제일은 정산 대상 기간을 결정하지 않습니다.", "서비스 이용 시작일·종료일, 지역·부서, 카드사, 예약번호·상품명 검색을 AND 조건으로 적용합니다. 위 요약 카드, 결제·정산 원장, 접힌 지역·부서·상품별 매출 요약과 엑셀 다운로드는 모두 같은 조회 결과를 사용합니다.", "시작일이 종료일보다 늦으면 조회 결과를 만들지 않고 오류를 표시하며 엑셀 다운로드를 비활성화합니다."], ["금액과 건수 산정", "조회 조건에 포함된 승인·취소 거래", "승인금액은 승인 거래의 합계, 취소금액은 전체·부분취소 거래 금액의 합계, 순매출은 승인금액에서 취소금액을 뺀 금액입니다.", "상단 카드는 승인금액·취소금액·수수료·지급예정액 순서로 표시합니다. 수수료는 거래별로 확인된 총수수료를 합산하되 취소에 따른 수수료 조정은 음수로 반영하고 ‘취소 수수료 조정 반영 완료’라고 안내합니다. 지급예정액은 순매출에서 총수수료를 뺀 금액이며 카드 아래에는 ‘순매출 - 수수료’라고 표시합니다.", "수수료 확인 대기·확인분·미확정 같은 별도 상태는 표시하지 않습니다. 지급예정액은 계산상 예정액이며 실제 입금 완료액을 뜻하지 않습니다."], ["결제·정산 원장과 매출 요약", "화면 조회 결과", "결제·정산 원장은 같은 예약번호의 승인·부분취소·전체취소를 한 행으로 묶고 예약번호·상품명, 서비스 이용일, 승인금액, 취소금액, 순매출, 수수료, 지급예정액과 지급예정일을 표시합니다. 제목 옆에는 결제 건수와 묶음 안의 원본 거래 건수를 함께 표시합니다.", "묶음 행을 펼치면 승인·취소 원본 거래를 최신순 타임라인으로 보존합니다. 각 이벤트에 거래일시, 구분, 거래금액, 결제수단, 카드사, 포트원 거래번호와 지역·담당부서를 표시하고, 취소 이벤트에는 취소된 인원별 티켓 번호와 취소사유를 함께 표시합니다. 이벤트별 수수료 문구는 표시하지 않고 묶음 행과 요약에서만 합산 결과를 제공합니다. 지역·부서·상품별 요약은 승인·취소 금액과 건수, 순매출, 수수료, 지급예정액을 집계하며 합계 행을 굵은 글씨와 상단 구분선으로 구분합니다.", "결제수단은 카드만 제공하며 간편결제와 계좌이체는 두지 않습니다. 주문번호·PG 기술키·카드번호·승인번호·할부·세금·면세금액·수수료 공급가액과 부가세는 정산 화면에서 제외합니다."], ["엑셀 다운로드 내용", "현재 조회 조건의 전체 결과", "화면과 동일한 서비스 이용일·지역·부서·카드사·예약번호·상품명 조건과 동일한 계산 결과를 사용합니다.", "‘월 정산’ 한 시트의 상단에 승인금액·취소금액·취소 제외 매출·수수료·입금예정액을 두고, 아래에는 예약번호별 서비스 이용일·상품명·카드사·취소 여부·각 금액과 입금예정일을 제공합니다.", "포트원 거래번호·취소 티켓 번호·거래일시·결제수단 등 원본 추적용 상세는 엑셀에서 제외하고 화면의 펼친 거래 이력에서 확인합니다. 결과가 0건이어도 조회 정보와 열 머리글이 있는 빈 엑셀을 생성합니다."], ["엑셀 표시 서식", "다운로드되는 월 정산 시트", "모든 셀은 11pt와 기본 행 높이를 사용합니다. 표 머리글은 회색, 합계 행은 연한 파란색으로 구분합니다.", "테두리는 실제 표 머리글과 데이터·합계 범위에만 적용하고 조회 정보, 안내 문구와 구분용 빈 행에는 적용하지 않습니다. 숫자는 천 단위 구분과 음수 표시를 사용하며 거래내역에는 틀 고정과 자동 필터를 제공합니다.", "행 높이나 글자 크기를 별도로 키우지 않고 줄무늬 배경을 사용하지 않습니다."], ["월 정산과 예외 환불", "서비스 이용일 기준 월 정산", "월 정산 대상은 해당 월에 실제 서비스가 완료된 건이며, 서비스 완료 전 부분취소를 함께 반영합니다. 미이용·서비스 전 전액취소 건은 제외합니다.", "포니체험은 결제 즉시 현장에서 완료되는 운영 특성을 반영합니다. 온라인 예약도 예약일이 아닌 실제 서비스 제공 완료일이 속한 월에 집계합니다.", "투어 프로그램의 정산 완료 후 환불 가능성은 브랜드총괄부 확인 사항입니다. 월 지급일과 예외 환불의 이전 달 소급 또는 다음 달 차감 기준은 확정 전까지 임의로 구현하지 않습니다."]],
    ...
    "ACC-01": [["계정 관리 접근", "통합 관리자", "부서 계정 발급과 다른 계정의 상태·권한 조회는 고객사 운영 관리자의 계정·권한 기능입니다.", "운영 관리자 메뉴에서 통합 관리자에게만 계정·권한 진입 경로를 제공합니다.", "통합 관리자만 계정 발급·재발급·계정 삭제를 수행합니다."]],
    "ACC-02": [["부서 공용 계정", "담당 부서별 1개", "지역과 담당 부서를 지정해 공용 계정을 발급하고 별도 통합 관리자 계정을 유지합니다. 프로필은 고정된 사람 아이콘과 부서명·관리자 권한 또는 ‘통합 운영 관리자’로 표시하며 개인 담당자 이름은 저장하거나 노출하지 않습니다.", "임시 비밀번호는 입력 중 확인할 수 있지만 저장 직후 입력값을 지우고 해시만 저장하며 다시 노출하지 않습니다.", "분실 시 기존 비밀번호 조회가 아닌 재발급만 허용합니다."]],
    "ACC-03": [["부서별 관리 · 조회", "프로그램·회차·운영일, 예약·결제, 취소·환불, 정산", "부서 계정은 자기 부서 프로그램을 관리하고 다른 부서 프로그램은 Read만 허용합니다.", "예약·환불·정산은 자기 부서만 접근하며 다른 부서의 변경·처리·다운로드 요청은 API에서 403으로 거절합니다.", "통합 운영 관리자는 모든 지역·부서 관리와 부서 계정 발급·삭제·재발급 권한을 가집니다."]],
  ...
  servicePolicy["ADM-07"][5][2] = "매월 1일부터 말일까지 실제 서비스 이용이 완료된 승인 거래를 해당 월의 정산 대상으로 묶습니다. 서비스 이용 전 전액취소 건도 예정 서비스 이용월에 포함해 승인금액과 취소금액을 표시하며 취소 제외 매출·수수료·입금예정액은 0원으로 표시합니다. 예약일과 결제일은 정산월을 결정하지 않습니다.";
  ...
  servicePolicy["ADM-07"].push(["취소 거래의 지급 표시", "결제·정산 원장에서 펼친 취소 거래 이력", "취소 거래 자체에는 새로 지급되는 금액과 지급일이 없습니다.", "묶음 행의 지급예정액에는 승인금액에서 취소금액과 수수료 조정을 반영한 결과를 표시하고, 펼친 거래 이력에는 취소금액만 표시합니다. 수수료 조정은 묶음 행의 수수료와 지급예정액 및 상단 요약에서 확인합니다.", "승인 거래의 지급예정일과 취소 거래의 정산 차감 의미를 혼용하지 않습니다."]);
  ...
  function topicsFor(context) {
    ...
    if (context === "account") return withService([
      ...
      { key: "ACC-04", title: "삭제 · 보안", summary: "프로그램·운영 이력에 따른 삭제 제한과 감사 이력", fe: [account[3]], be: [adminApi[5], adminApi[6], adminApi[7]] }
    ]);
    if (context === "admin") return withService([
      ...
      { key: "ADM-07", title: "매출 · 정산과 엑셀", summary: "서비스 이용일 기준 조회부터 요약·거래내역·엑셀 산식과 표시 범위까지", fe: [admin[9]], be: [adminApi[4]] },
    ...
    return withService([
      ...
      { key: "RSV-03", title: "회차 · 잔여 수량", summary: "프로그램 간 시간 중복 제한과 잔여 좌석 판정", fe: [customer[3]], be: [bookingApi[1], bookingApi[6]] },
      ...
      { key: "RSV-07", title: "예약 티켓", summary: "회차별 티켓 표시, 상태 배지와 전체·부분취소·입장 처리", fe: [customer[10], customer[11], customer[12]], be: [bookingApi[5], bookingApi[7], bookingApi[8], bookingApi[10]] },
